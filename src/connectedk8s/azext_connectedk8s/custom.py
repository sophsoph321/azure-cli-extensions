...
import base64
from ._client_factory_troubleshoot import cf_storage
...
def check_helm_installed(condition, failMsg, passMsg):
...
def check_helm_3(condition, failMsg, passMsg):
    try:
...
def collect_logs(cmd, client, resource_group_name, name, storage_account=None,
    ...
    kube_objects = "azure-arc/pod azure-arc/service azure-arc/deployment azure-arc/crd"
...
def troubleshoot(cmd, client, resource_group_name, name, storage_account=None,
                node_logs=None,
                sas_token=None,
                container_logs=None,
                kube_objects=None,
    ...
    troubleshoot_file = os.path.join(os.path.expanduser('~'), '.azure', 'cliextensions', 'connectedk8s', 'azext_connectedk8s', 'troubleshoot.json')
    platform = ''
    try:
        #with open('troubleshoot.json', 'r') as f:
        with open(troubleshoot_file, 'r') as f:
            checks = json.load(f)
        for check in checks:
            if check['Enabled']:
                method = eval(check["Method"]+'(check["Condition"], check["FailMessage"], check["PassMessage"])')
                method
    except OSError as e:
        print(e)
        print("Please ensure troubleshoot.json is in local directory.")
    
    collect_logs(cmd, client, resource_group_name, name, storage_account, sas_token, container_logs, kube_objects, node_logs, )
